...
```{r Q3}
createWeights <- function(df, ctr.wts) {
    # Create weights variable for 1-1 matching to treatment group with replacement
    df[df$treat==0, 'wt'] <- ctr.wts
    df[df$treat==1, 'wt'] <- 1
    return(df)
}

matches <- matching(z=dt$treat, score=dt$psc, replace=TRUE)
dt <- createWeights(dt, matches$cnts)
```
...
# Overlap of days child in hospital after birth
examine_overlap(dt, 40, var='dayskidh', var_name='Days child in hospital after birth', xlab='Days', ylim=c(-60, 80))
```
...
>>>>>>> da6c3b3be817a071c3ff35a8861ccbbe473330ec
...
\texttt{ANSWER:}
```{r Q4d}
checkBalance(df=dt, confounders=confounders, wt_var='wt')
```
...
temp <- createWeights(temp, ps.m2.matches$cnts)
...
myMH = function(trtnms, ctrnms, inv.cov, data){
    ...
    xdiffs <-as.matrix(data[trtnms, covars]) - as.matrix(data[ctrnms, covars])
...
mahaloMahalanobis = function(df, confounders) {
    ...
    inv_cov_mat = solve(cov(df[, confounders]))
...
matchMahalanobis = function(mahalanobisDist, data) {
    # Find nearest neighbor given mahalanobis distance matrix; return weight counts
    
    treat.idx = as.integer(row.names(data[data$treat==1,]))
    ctrl.idx = as.integer(row.names(data[data$treat==0,]))
    # Find matched pairs
    matches = apply(mahaloDist, 1, function(d) min(which(d == min(d))))
    match.results = data.frame(trt.idx=treat.idx,
        ctr.idx=matches + length(treat.idx))
    # Count match weights for controls
    counts = as.data.frame(tapply(match.results$ctr.idx, match.results$ctr.idx, length))
    # Append unmatched controls
    ctrls = as.integer(row.names(data[data$treat==0,]))
    rows = ctrls[!(ctrls %in% row.names(counts))]

    which(!(row.names(data[data$treat==0,]) %in% as.integer(row.names(counts)))]
    unmatched = data.frame()
    return(counts)
}

mahalanobisDist = mahaloMahalanobis(dt, confounders)
dt[dt$treat==0, 'w.mh'] = matchMahalanobis(mahalanobisDist)
dt[dt$treat==1, 'w.mh'] = 1
# Find nearest neighbor using mahalanobis distance
matches <- apply(mahalanobisDist, 1, function(x) min(which(x == min(x))))
match_results <- data.frame(trt.idx=seq(1,290), ctr.idx=matches+290))
