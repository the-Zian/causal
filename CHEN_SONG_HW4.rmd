...
We compare the mean differences and ratios of standard deviations among confounders between the treatment and control groups to assess which propensity and/or matching approach results in the best balance. We aim to reduce the mean differences to below 0.10 for continuous variables and 0.05 for binary variables. For the ratio of standard deviations, we aim to get as close to 1.0 as possible. Considering the number of confounders and approaches, the above tables can make assessments of which approach is best due to the number of values to compare. The plots from Question 5 aid in comparing mean differences and ratios of standard deviations.  
We prioritize balancing mean differences over ratios because we only consider 3 continuous confounders, and none of the approaches are significantly better in balancing the ratios. The Mahalanobis matching approach achieved the best balance in mean differences, and thus we choose this approach.
...
\texttt{ANSWER:}  
For children who participated in the IDHP intervention, their IQ scores at age 3 were `r round(mod4.out[1], 1)` points higher than had they not participated in the intervention.
...
\texttt{ANSWER:}  
A group of children who participate in the IHDP intervention will expect, on average, an 11.54 point increase in their age 3 IQ scores compared to a group of children who did not participate in the intervention, controlling for all other covariates.
...
\texttt{ANSWER:}  
In terms of estimating causal effects, propensity score approaches are preferred over linear regression to account for the systematic differences between the treatment and control groups in key covariates that can affect the outcome. While linear regression can account for some group differences, the method is highly dependent on correct model specificity in order avoid treatment effect estimation bias. Propensity score approaches decrease this bias by restructuring the sample of controls and/or treated observations so that there is overlap in the common support of the covariates between groups, and improved balance in distributions of the covariates between groups. Propensity score approaches decrease the treatment effect estimation bias caused by extrapolating into areas of the covariate space outside of the support of the data, and incorrect model specification of the treatment and covariates on the outcome.
