...
- indicators for child's race/ethnicity (black, hispanic, white)
...
#####################################################  
Your response below is unclear. It descirbes that target of the intervention. The research question could be interested in the effect of the intervention on average child in America even if the target of the intervention was children born premature.
## 
...
examine_overlap(dt, nbins=40, var='psc', var_name='Propensity Score', xlab='Propensity Score', ylim=c(-30,40))
# Overlap of preterm
...
```{r Q4d plot}
q4df = data.frame(ps.m1.bal[,c(5,6)])
q4df$var = as.factor(row.names(q4df))
colnames(q4df) = c('unmatched','matched','var')

q4df = melt(q4df)

ggplot(q4df, aes(x = value, y = var, shape = variable, color = variable)) + 
  geom_point() + 
...
matches_nr_idx <- c(which(dt$treat==1), 
                    matches_no_replace$matched[1:290])
