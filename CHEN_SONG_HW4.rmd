...
# St99: indicator for whether family was living in state served by the ihdp
# Also, all subjects with St99=1 were in control
apply(hw4[hw4$treat==1,],2,sum)
confounders <- confounders[!(confounders %in% c('st99'))]
...
We excluded 'st99' because all subjects with St99 = 1 were in the control group.
...
#####################################################  
Alan, wanted for you to see why your response below is unclear. You describe the target of the intervention then state, "thus estimand of interest is ATT". But the research question is independent of the target of the intervention. For example, the research question could be interested in what the effect of the intervention would be on the average American, even though the intervention itself targeted premature children.
...
plotMeanDiff = function(balance_object){
  ...
  ggplot(df, aes(x = value, y = var, shape = variable, color = variable)) + 
    ...
    labs(x = 'Standardized Difference in Means', y = 'predictor')
}

plotMeanDiff(ps.m1.bal)
```
...
nn_mod = nnet(treat ~ ., data=dt_nnet, size=12, linout=FALSE, maxit=500, decay=0.1)
...
=======
>>>>>>> 3f201f53c0794d8bf248d548150c17b1e7335f4d
...
# Set Weights
dt[dt$treat==0,'wt.mh'] = matches$Freq
...
# Check control subject ID of our Mahalanobis matches to the MatchIt matches
my_ctrl_idx = row.names(dt[dt$treat==0 & dt$wt.mh>0,])
matchIt_ctrl_idx = zz$match.matrix

my_ctrl_idx %in% matchIt_ctrl_idx 
```
...
plotMeanDiff(checkBalance(dt, confounders, 'wt'))  
plotMeanDiff(checkBalance(dt, confounders, 'w_probit'))  
plotMeanDiff(checkBalance(dt, confounders, 'w_full'))  
plotMeanDiff(checkBalance(dt, confounders, 'w_logit_no_rep'))  
plotMeanDiff(checkBalance(dt, confounders, 'w_rf'))  
plotMeanDiff(checkBalance(dt, confounders, 'w_nn'))  
plotMeanDiff(checkBalance(dt, confounders, 'wt.mh'))  
